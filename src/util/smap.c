...
void print_smap(smap* map){
    for (size_t i = 0; i < map->num_buckets; i += 1) {
        for (size_t j = 0; j < map->buckets[i].num_pairs; j += 1) {
            if (strstr(map->buckets[i].pairs[j].key, "    struct.") != NULL
                && strstr(map->buckets[i].pairs[j].key, "    .struct") == NULL) {
                char* zero_len = safe_calloc(sizeof(char) * (map->buckets[i].pairs[j].val * 2 + 1));
                for (int n = 0; n < map->buckets[i].pairs[j].val; n += 1){
                    strcat(zero_len, "0 ");
                } 
                printf("    %s: .word %s\n", map->buckets[i].pairs[j].key, zero_len);
                free(zero_len);
            } else {
                printf("    %s: .word %d\n", map->buckets[i].pairs[j].key, map->buckets[i].pairs[j].val);
            }
        }
    }
}